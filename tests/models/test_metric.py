...
def test_levenshtein_distance():
    str1 = "kitten"
    str2 = "sitting"
    distance_metric = LevenshteinDistance(str1, str2, threshold=2)
    distance = distance_metric.compute()
    assert isinstance(distance, int)
    assert distance_metric.passes_threshold is False

    distance_metric_threshold = LevenshteinDistance(str1, str2, threshold=4)
...
def test_fuzzy_ratio():
    str1 = "kitten"
    str2 = "sitting"
    ratio_metric = FuzzyRatio(str1, str2, threshold=90.0)
    ratio = ratio_metric.compute()
    assert isinstance(ratio, float)
    assert ratio_metric.passes_threshold is False, (
        f"Expected False but got True for ratio {ratio} with threshold 90.0"
    )

    ratio_metric_threshold = FuzzyRatio(str1, str2, threshold=50.0)
    ratio = ratio_metric_threshold.compute()
    assert ratio_metric_threshold.passes_threshold is True, (
        f"Expected True but got False for ratio {ratio} with threshold 50.0"
    )
...
def test_metric_with_word_objects():
    word1 = Word("hello")
    word2 = Word("hallo")
    word_obj1 = FuzzyRatio(word1, word2, threshold=80.0)
